...
export default class MysInfo {
  ...
  /**@param {{e,apis,game,option?:{cached?, cacheCd?, log?}}} params */
  static async init (params = {}) {
    const { e = '', apis = '', game = '', option = {} } = params
    ...
    if (mysInfo.ckInfo?.ck) {
      mysInfo.mysApi = new MysApi({ ...mysInfo.ckInfo, uid: mysInfo.uid, game: mysInfo.game }, option)
    }
    return mysInfo
  }
  ...
  async getData (api, data) {
    ...
    return await this.checkCode(ret, api, data)
  }
