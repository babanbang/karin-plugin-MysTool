...
export default class MysInfo {
  constructor (e = {}, game = '') {
    ...
    this.game = MysUtil.getGameByMsg(e.msg).key || game
    ...
    if (Array.isArray(apis)) {
      ...
      if (e.apiSync) {
        ...
        if (ret?.retcode === 0) {
          ...
          const all = []
          _apis.forEach(v => all.push(mysApi.getData(...v)))
  ...
  async checkCode (res, type, mysApi = {}, data = {}, isTask = false) {
    ...
    switch (res.retcode) {
      ...
      case 10035:
        ...
        if (!isTask) this.e.reply(`UID:${this.uid}，米游社查询遇到验证码，请稍后再试`)
        break
      case 10307:
