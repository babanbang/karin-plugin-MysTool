...
export default class MysInfo {
  ...
  setMysApi (mys = {}, option = {}) {
    if (lodash.isEmpty(this.ckInfo)) this.ckInfo = mys
  ...
  /**@param {{e,apis,game,option?:{cacheCd?, log?},UidType?:'ck'|'sk'}} params */
  static async init (params = {}) {
    const { e = '', apis = '', game = '', option = {}, UidType = '' } = params
    ...
    if (!MysUtil.matchUid(String(mysInfo.uid), mysInfo.game)) {
      e.reply('查询UID错误，请检查UID是否正确')
      return false
    }
    if (!mysInfo.hoyolab && apis === 'useCdk') {
  ...
  static async getCksByGame (e, game) {
    const user = await User.create(e, game)
    return user.getUidsData()
  }
  ...
  static async get (e, apis, data = {}, option = {}) {
    const mysInfo = await MysInfo.init({ e, apis, option, UidType: data.UidType || option.UidType, game: data.game || option.game })
    ...
    if (Array.isArray(apis)) {
      ...
      if (data.apiSync || option.apiSync) {
        ...
        if (ret?.retcode === 0) {
          ...
          const all = []
          const _apis = lodash.without(apis, api)
  ...
  async getData (api, data, check = true) {
  ...
  async getCookie (uid) {
    ...
    if (mysUser) {
      this.ckInfo = mysUser.getLtuidData()
      this.ckUser = mysUser
    }
    return false
  }
    ...
    switch (res.retcode) {
      ...
      case 10103:
        if (/(登录|login)/i.test(res.message)) {
          if (!this.ckInfo.isUp && !data.useSK) {
            this.mysUser = await MysUser.delCK(this.ckInfo.ltuid)
            this.mysApi.cookie = this.ckInfo.ck = this.mysUser.ck
            this.ckInfo.isUp = true

            if (this.ckInfo.ck) return await this.getData(type, data)
          }

          if (this.UidType == 'sk' || data.useSK) await MysUser.delSK(this.ckInfo.ltuid)

          if (!this.ckInfo.ck || this.ckInfo.isUp) {
            if (this.ckInfo.uid) {
              logger.mark(`[${(this.UidType == 'sk' || data.useSK) ? 'Stoken' : 'Cookie'}失效][uid:${this.uid}][qq:${this.user_id}]`)
              if (!isTask) this.e.reply(`UID:${this.ckInfo.uid}，米游社${(this.UidType == 'sk' || data.useSK) ? 'Stoken' : 'Cookie'}已失效`)
            } else {
