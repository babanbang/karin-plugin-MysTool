...
export default class MysApi {
   ...
   */
  constructor (
    { uid = '', user_id = '', cookie = '', ck = '', sk = '', game = 'gs', server = '', device = '' } = {},
    option = {}
  ) {
    this.uid = uid
    ...
    this.stoken = sk
    this.game = game
    this.server = server || MysUtil.getRegion(this.uid, this.game)
    this.device_id = device || MysUtil.getDeviceGuid()
  ...
  async getData (type, data = {}) {
    ...
    // logger.error(`[米游社接口][${type}][${this.game}][${this.user_id || this.uid}] ${JSON.stringify(res)}`)
