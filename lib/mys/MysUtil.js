import { lodash, moment } from 'node-karin/modules.js';
import { game_biz, game_region } from './MysTool.js';
export const MysUtil = new (class Mysutil {
    #all_game_biz = Object.values(game_biz).flat();
    #gamelist = [];
    #allgames = [
        { key: "gs" /* GameList.Gs */, name: "\u539F\u795E" /* GameNames.Gs */ },
        { key: "sr" /* GameList.Sr */, name: "\u5D29\u574F\uFF1A\u661F\u7A79\u94C1\u9053" /* GameNames.Sr */ },
        { key: "zzz" /* GameList.Zzz */, name: "\u7EDD\u533A\u96F6" /* GameNames.Zzz */ }
    ];
    get reg() {
        return {
            ["sr" /* GameList.Sr */]: '(\\*|#?(sr|星铁|星轨|穹轨|星穹|崩铁|星穹铁道|崩坏星穹铁道|铁道))',
            ["zzz" /* GameList.Zzz */]: '(%|#?(zzz|绝区零))',
            ["gs" /* GameList.Gs */]: '#?(gs|原神)'
        };
    }
    get servs() { return ['mys', 'hoyolab']; }
    get games() { return [...this.#gamelist]; }
    get AllGameBiz() { return [...this.#all_game_biz]; }
    get EndOfDay() {
        return Number(moment().endOf('day').format('X')) - Number(moment().format('X'));
    }
    addGame(key, name) {
        this.#gamelist.push({ key, name });
        this.#gamelist = lodash.sortBy(this.#gamelist, 'key');
    }
    getGame(game) {
        return this.games.find((g) => g.key === game);
    }
    getGameByMsg(msg = '') {
        for (const i in this.reg) {
            const game = i;
            if (new RegExp('^' + this.reg[game]).test(msg)) {
                return this.games.find((g) => g.key === i);
            }
        }
        return this.games[0];
    }
    getServ(uid, game) {
        return (game === "zzz" /* GameList.Zzz */ ? /^(10|13|15|17)[0-9]{8}/i : /^(18|[6-9])[0-9]{8}/i).test(uid) ? this.servs[1] : this.servs[0];
    }
    getGamebiz(game, os = false) {
        return game_biz[game][os ? 1 : 0];
    }
    getGameByGamebiz(biz) {
        for (const i in game_biz) {
            const game = i;
            if (game_biz[game].includes(biz)) {
                return game;
            }
        }
        return "gs" /* GameList.Gs */;
    }
    getGameByRegion(region) {
        for (const i in game_region) {
            const game = i;
            if (game_region[game].some(r => r.region === region)) {
                return game;
            }
        }
        return "gs" /* GameList.Gs */;
    }
    getRegion(uid, game) {
        if (game == "zzz" /* GameList.Zzz */) {
            if (uid.length < 10) {
                return game_region[game][0].region; // 官服
            }
            switch (uid.slice(0, -8)) {
                case '10':
                    return game_region[game][2].region; // 美服
                case '15':
                    return game_region[game][3].region; // 欧服
                case '13':
                    return game_region[game][4].region; // 亚服
                case '17':
                    return game_region[game][5].region; // 港澳台服
            }
        }
        else {
            switch (uid.slice(0, -8)) {
                case '5':
                    return game_region[game][1].region; // B服
                case '6':
                    return game_region[game][2].region; // 美服
                case '7':
                    return game_region[game][3].region; // 欧服
                case '8':
                case '18':
                    return game_region[game][4].region; // 亚服
                case '9':
                    return game_region[game][5].region; // 港澳台服
            }
        }
        return game_region[game][0].region; // 官服
    }
    getCookieMap(cookie) {
        const cookieArray = cookie.replace(/#|'|"/g, '').replace(/\s*/g, '').split(';');
        const cookieMap = {};
        for (let item of cookieArray) {
            const entry = item.replace('=', '~').split('~');
            if (!entry[0])
                continue;
            cookieMap[entry[0]] = entry[1];
        }
        return cookieMap;
    }
    splicToken(token) {
        return Object.entries(token).filter(([k, v]) => v).map(([k, v]) => `${k}=${v}`).join(';') + ';';
    }
    randomString(n) {
        return lodash.sampleSize('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', n).join('');
    }
    isHoyolab(region, game) {
        return game_region[game].find((r) => r.region === region).os;
    }
    matchUid(msg, game) {
        const uid = msg.match(game === "zzz" /* GameList.Zzz */ ? /((10|13|15|17)[0-9]{8}|[1-9][0-9]{7})/g : /((18|[1-9])[0-9]{8})/g);
        if (!uid?.[0] || uid.length > 1)
            return false;
        return uid[0];
    }
    /** 循环game */
    async eachGame(fn, all = false) {
        for (const ds of (all ? this.#allgames : this.games)) {
            await fn(ds.key, ds);
        }
    }
    /** 生成设备guid */
    getDeviceGuid() {
        function S4() {
            return (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1);
        }
        return (S4() + S4() + '-' + S4() + '-' + S4() + '-' + S4() + '-' + S4() + S4() + S4());
    }
    getSeedId(n) {
        return lodash.sampleSize('0123456789abcdef', n);
    }
    RegionName(region, game) {
        if (Number(region))
            region = this.getRegion(region, game);
        return game_region[game].find((r) => r.region === region).name;
    }
    checkMonth(year, month, num = 2) {
        month -= 1;
        const nowDate = new Date();
        let nowYear = nowDate.getFullYear();
        let nowMonth = nowDate.getMonth();
        nowDate.setMonth(nowDate.getMonth() - num);
        let twoMonthsAgoYear = nowDate.getFullYear();
        let twoMonthsAgoMonth = nowDate.getMonth();
        if (year > twoMonthsAgoYear || (year === twoMonthsAgoYear && month >= twoMonthsAgoMonth)) {
            if (year < nowYear || (year === nowYear && month <= nowMonth)) {
                return true;
            }
        }
        return false;
    }
});
