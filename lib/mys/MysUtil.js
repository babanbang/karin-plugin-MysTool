...
const MysUtil = {
  ...
  addGames (key, name) {
    ...
    this.games = lodash.sortBy(this.games, 'key')
  },
  servs: ['mys', 'hoyolab'],
  getServ (uid, game) {
    return (game == 'zzz' ? /^(11|12|13|14)[0-9]{8}/i : /^(18|[6-9])[0-9]{8}/i).test(uid) ? this.servs[1] : this.servs[0]
  },
  ...
  async eachGame (fn, all = false) {
    await Data.forEach([...(all ? this.allgames : this.games)], (ds) => {
      return fn(ds.key, ds)
    })
  },
  /** 生成设备guid */
  ...
  matchUid (msg, game) {
    const uid = msg.match(game === 'zzz' ? /((11|12|13|14)[0-9]{8}|[1-9][0-9]{7})/g : /((18|[1-9])[0-9]{8})/g)
  ...
  getGameByRegion (region) {
    for (const game in game_region) {
      if (game_region[game].find((r => r.region == region))) {
        return game
      }
    }
    return ''
