...
import { Artifact, Avatar, Character, Format, Serv } from './index.js'
...
export default class Player extends Base {
  ...
  save (flag = null) {
    ...
    const ret = Data.getData(this, 'uid,name,level,face,card,world_level')
    ret.avatars = {}
    this.forEachAvatar(
      /** @param {Avatar} avatar */
      (avatar) => {
  ...
  setBasicData (ds, save = false) {
    ...
    if (this.game == 'zzz') {
      this.world_level = ds.world_level || this.world_level || ''
    }
    ...
    this.card = String(ds.card || this.card || '').replace(/\.png$/, '')
  ...
  getAvatar (id, create = false) {
    const char = Character.get(id, this.game)
  ...
  async getProfileDetail ({ name, mode = 'dmg', dmgIdx = '' }) {
    ...
    lodash.forEach((this.isGs ? 'hp,def,atk,mastery' : 'hp,def,atk,speed').split(','), (key) => {
      let fn = (n) => Format.comma(n, key === 'hp' ? 0 : 1)
      attr[key] = fn(a[key])
      attr[`${key}Base`] = fn(base[key])
      attr[`${key}Plus`] = fn(a[key] - base[key])
    })
    lodash.forEach((this.isGs ? 'cpct,cdmg,recharge,dmg,heal' : 'cpct,cdmg,recharge,dmg,effPct,effDef,heal,stance').split(','), (key) => {
      let fn = Format.pct
      let key2 = key
