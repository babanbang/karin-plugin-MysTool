...
export default class Character extends Base {
  ...
  constructor ({ id, data = {}, name = '', elem = '', game = 'gs' }) {
    ...
    if (!this.isCustom) {
      ...
      if (this.isGs) {
        this.meta.elem = Format.elem(elem || this.meta.elem, game, 'anemo')
      }
    } else {
      this.meta = {}
    }
  ...
  /** 是否为官方角色 */
  get isOfficial () {
    return ['zzz', 'sr'].includes(this.game) || /[12]0\d{6}/.test(this._id)
  }
  ...
  get element () {
    return Format.elem(this.elem, this.game)
  }

  // 获取元素名称
  get elemName () {
    if (this.isSr || this.isZzz) {
      return this.elem
    }
  ...
  get vertical () {
    return `meta/character/${this.name}/imgs/vertical.png`
  }
  ...
  getAvatarTalent (talent = {}, cons = 0, mode = 'original') {
    ...
    lodash.forEach(Talent[this.game], (addNum, k) => {
      const ds = talent[k]
      if (!ds) return false

      const value = lodash.isNumber(ds) ? ds : (ds.original || ds.level_original || ds.level || ds.level_current)
      ...
      ret[k].id = this.detail?.talent?.[k]?.id
  ...
  getDetail () {
    ...
    try {
      ...
      this.meta._detail = Data.readJSON(`lib/components/${Data.gamePath(this.game)}/resources/meta/character/${name}/data.json`)

      if (this.id == 10000005) this.meta._detail.talent.a.id -= 1
    } catch (e) {
      logger.error(e)
    }
    return this.meta._detail
