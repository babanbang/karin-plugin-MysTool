...
import { Artifact, Format, Meta } from '#MysTool/profile'

let ArtisMark = {
  getKeyTitleMap (game = 'gs') {
    let ret = {}
    let { attrMap } = Meta.getMeta(game, 'arti')
    _.forEach(attrMap, (ds, key) => {
      ret[key] = ds.title
    })
  ...
  getMarkDetail (profile, withDetail = true) {
    ...
    if (withDetail) {
      ret.charWeight = _.mapValues(charCfg.attrs, ds => ds.weight)
    }
    return ret
  }
}
