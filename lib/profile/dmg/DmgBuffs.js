...
const DmgBuffs = {
  getBuffs (profile, buffs = [], game = 'gs') {
    ...
    buffs = _.concat(buffs, weaponBuffs, artisBuffs)
    let mKey = {
      vaporize: '蒸发',
      melt: '融化',
      swirl: '扩散'
    ...
    buffs = _.filter(buffs, (b) => !!b)
    _.forEach(buffs, (buff, idx) => {
      if (_.isString(buff)) {
        if (mKey[buff]) {
          buff = {
      ...
      buff.sort = _.isUndefined(buff.sort) ? 1 : buff.sort
    })
    buffs = _.sortBy(buffs, ['sort'])
    return buffs
  },
  // 圣遗物Buff
