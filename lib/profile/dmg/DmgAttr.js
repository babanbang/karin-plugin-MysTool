...
* 伤害计算 - 属性计算
* */
import { Format, Meta } from '#MysTool/profile'
...
const keys = {
  gs: ['mastery,recharge,cpct,cdmg,heal,dmg,phy'.split(','), 'a,a2,a3,e,q'.split(',')],
  sr: ['speed,recharge,cpct,cdmg,heal,dmg,enemydmg,effPct,effDef,stance'.split(','), 'a,a2,a3,e,e2,q,q2,t,dot,break'.split(',')]
}
const DmgAttr = {
  // 计算并返回指定属性值
  getAttrValue (ds = {}) {
  ...
  getAttr ({ originalAttr, attr, weapon, char, game = 'gs' }) {
    const ret = _.merge({}, originalAttr || {})

    // 基础属性
    _.forEach('atk,def,hp'.split(','), (key) => {
  ...
  calcAttr ({ originalAttr, buffs, meta, artis, params = {}, incAttr = '', reduceAttr = '', talent = '', game = 'gs' }) {
    const attr = DmgAttr.getAttr({ originalAttr, game })
    const msg = []
    const { attrMap } = Meta.getMeta(game, 'arti')
    ...
    _.forEach(buffs, (buff) => {
      const ds = DmgAttr.getDs(attr, meta, params)
      ...
      if (buff.cons && ds.cons * 1 < buff.cons * 1) {
        return
      }
      if (!_.isUndefined(buff.maxCons) && ds.cons * 1 > buff.maxCons * 1) {
        return
      }
      if (buff.tree && !ds.trees[`10${buff.tree}`]) {
        return
      }
      ...
      _.forEach(buff.data, (val, key) => {
        ...
        // 技能提高
        let tRet = /^(a|a2|a3|e|q|t|dot|break|superBreak)(Def|Ignore|Dmg|Enemydmg|Plus|Pct|Cpct|Cdmg|Multi)$/.exec(key)
        if (tRet) {
