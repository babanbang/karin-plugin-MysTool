...
const DmgAttr = {
  ...
  getAttr ({ originalAttr, attr, weapon, char, game = 'gs' }) {
    const ret = lodash.merge({}, originalAttr || {})
    ...
    lodash.forEach(keys[game][0], (key) => {
      ret[key] = AttrItem.create(originalAttr?.[key] || {
    ...
    // 技能属性记录
    lodash.forEach(keys[game][1], (key) => {
      ret[key] = ret[key] || {
        pct: 0, // 倍率加成
  ...
  calcAttr ({ originalAttr, buffs, meta, artis, params = {}, incAttr = '', reduceAttr = '', talent = '', game = 'gs' }) {
    ...
    lodash.forEach(buffs, (buff) => {
      ...
      if (!lodash.isUndefined(buff.maxCons) && ds.cons * 1 > buff.maxCons * 1) {
        return
      }
