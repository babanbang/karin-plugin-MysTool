...
import { Character, Format, ProfileDmg, Weapon } from './index.js'

export default class Avatar extends Base {
  constructor (ds = {}, game = 'gs') {
    super(game)
    let char = Character.get({ id: ds.id, elem: ds.elem }, game)
    if (!char) return
  ...
  get artifact () {
    const artis = []
    lodash.forEach(this.Artis.artis, (v, k) => {
      if (!lodash.isEmpty(v) && v.name && v.set) {
        artis.push({
          artiskey: k,
          ...v
        })
  ...
  setBasic (ds = {}, source = '') {
    ...
    this.elem = Format.elem(this.char.elem, this.game).toLowerCase()
  ...
  setAvatar (ds, source = '') {
    ...
    if (!lodash.isEmpty(ds.artis) && source !== 'mys') {
  ...
  setAvatarBase (ds, source = '') {
    ...
    if (!lodash.isEmpty(ds.talent)) this.setTalent(ds.talent, 'original', source)
  ...
  setWeapon (ds = {}) {
    ...
    this.weapon = {
      ...
      promote: lodash.isUndefined(ds.promote || ds.promote_level) ? Attr.calcPromote(ds.level || ds.lv || 1) : (ds.promote || ds.promote_level || 0),
  ...
  setTreesAct (data) {
    data.talent_trees = []
    lodash.forEach(this.char.detail.tree, (v, id) => {
