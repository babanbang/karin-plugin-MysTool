...
export class MysUser extends base {
  ...
  /**@returns {{ltuid, type:'mys'|'hoyolab', ck: string, sk: string, device: string}} */
  getLtuidData (list = false) {
    return Data.getData(this, list || 'ltuid,type,ck,sk,device')
  }
 ...
 */
  ...
  */
  ...
  addUid (uid, game = '') {
    ...
    if (game == 'zzz' ? /\d{8,10}/.test(uid) : /\d{9,10}/.test(uid)) {
  ...
  static async delCK (ltuid) {
    const mys = await MysUser.create(ltuid)
    mys.ck = await mys.updataCookie([mys.type])
    await mys.save()
    return mys
  }

  static async delSK (ltuid) {
    const mys = await MysUser.create(ltuid)
    mys.sk = ''
    mys.stoken = ''
    await mys.save()
