...
class Cfg {
  ...
  initCfg (Path = '', PathName = '', game = '') {
    ...
    const configPath = `${dirPath}${Path}${PathName}/config/config/`
    const defSetPath = `${dirPath}${Path}${PathName}/config/defSet/`
    if (!fs.existsSync(defSetPath)) return
    ...
    Data.createDir(`data/${PluginName}/${PathName}`, true)
  }
