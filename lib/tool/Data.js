...
const Data = {
  getFilePath (file, Path) {
    if (Path === 'root') return `./data/${PluginName}/` + file.replace(/^\//g, '')
    if (Path) return getDir(Path).path + '/' + file.replace(/^\//g, '')
    return dirPath + '/' + file.replace(/^\//g, '')
  },
  isDirectory (path, Path = '') {
    const _path = this.getFilePath(path, Path)
    return fs.statSync(_path).isDirectory()
  },
  /**@param {Array} dir  */
  createDirs (dir, root = false) {
    dir.forEach(i => {
      Data.createDir(i, root)
    })
  },
  ...
  readdir (dir, Path = '') {
    const path = this.getFilePath(dir, Path)
  ...
  createDir (path, root = false) {
    let file = '/'
    let dirpath = root ? `./data/${PluginName}` : dirPath
  ...
  * */
  ...
  /** 
   * 写入json
   * @param {{rn?: boolean, space?: number, root?: boolean}} cfg 
  */
