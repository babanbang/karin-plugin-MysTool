...
import { dirPath, getDirPath } from '../../index.js'

const Data = {
  getFilePath (file, Path) {
    if (Path) return getDirPath(Path) + '/' + file.replace(/^\//g, '')
  ...
  * */
  ...
  gamePath (game) {
    return game === 'gs' ? 'Genshin/' : game === 'sr' ? 'StarRail/' : game === 'zzz' ? 'Zenless/' : ''
  },
  /** 循环字符串回调 */
  eachStr: (arr, fn) => {
    if (_.isString(arr)) {
  ...
  async importModule (file, Path = '') {
    ...
    if (fs.existsSync(path)) {
      try {
        return await import(`file://${path}?t=${new Date() * 1}`) || {}
      } catch (e) {
        logger.error(e)
      }
    }
