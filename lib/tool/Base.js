...
export default class Base {
  ...
  async renderImg (data, cfg = {}) {
    if (data && cfg.test) {
      Data.writeJSON(`data/${PluginName}/html/${this.model}/${this.game}/data.json`, data, { root: true })
    }
    const ImageData = {
      name: this.ModelName,
      ...
      data: {
        ...
        PluginName,
        ...((!data && cfg.test)
          ? Data.readJSON(`data/${PluginName}/html/${this.model}/${this.game}/data.json`, 'root')
          : data),
        sys: {
          scale: cfg.scale || 1
        }
