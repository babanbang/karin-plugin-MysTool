 ...
 */
export async function wkhtmltoimage (renderData) {
  const { name, quality } = renderData
...
/** 模板 */
function dealTpl (name, renderData) {
  const { file, fileID = name, data } = renderData
  data.useBrowser = '-wk'
  const path = `temp/html/${name.replace(new RegExp(`${PluginName}/`, 'g'), '')}/${fileID}.html`
