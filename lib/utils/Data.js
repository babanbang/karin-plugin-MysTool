...
import { PluginName, dirPath, getDir } from '../../index.js'

const GamePath = { gs: 'Genshin/', sr: 'StarRail/', zzz: 'Zenless/', sign: 'MysSign/' }
...
const Data = {
  ...
  getFilePath (file, cfg = {}) {
    if (cfg?.temp) return KarinPath + `/temp/${PluginName}/` + file.replace(/^\//g, '')
    if (cfg?.root) return KarinPath + `/data/${PluginName}/` + file.replace(/^\//g, '')
  ...
  */
  ...
  createDir (path, cfg = {}) {
    let file = '/'
    let dirpath = cfg?.temp ? (KarinPath + `/temp/${PluginName}`)
      : cfg?.root ? (KarinPath + `/data/${PluginName}`) : dirPath
  ...
  * */
  getData (target = {}, keys = '', def = {}) {
    ...
    lodash.forEach(keys, (key) => {
      ...
      ret[_key[0].trim()] = lodash.get(target, (_key[1] || _key[0]).trim(), def[key])
    })
    return ret
  },
  /** 
