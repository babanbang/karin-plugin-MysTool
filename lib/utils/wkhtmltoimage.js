...
import { Cfg, Data, PluginName, dirPath } from '#MysTool/utils'
...
async function load () {
  if (!set.wkhtmltoimage || !port) return
  ...
  async function serverEADDRINUSE () {
    logger.error(`[wkhtmltoimage] http://localhost:${port} 被占用`)
    try {
...
if (set.wkhtmltoimage && port) await load()
 ...
 */
export async function wkhtmltoimage (data) {
  if (!wk) logger.error(PluginName + '未安装wkhtmltoimage，请安装后重启')
  if (!initwkhtmltoimage) await load()
