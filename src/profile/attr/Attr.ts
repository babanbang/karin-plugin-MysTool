...
import { Format, Meta, Weapon, ArtifactSet } from '#MysTool/profile'
...
export default class Attr extends Base {
  ...
  setCharAttr () {
    ...
    _.forEach(charBuffs.buffs, (buff) => {
  ...
  setWeaponAttr () {
    ...
    if (this.isSr) {
      ...
      if (weapon.type === char.weapon) {
        ...
        _.forEach(wBuffs, (buff) => {
          _.forEach(buff.data || [], (v, k) => {
            self.addAttr(k, v)
          })
        })
      }
