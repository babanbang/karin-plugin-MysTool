...
const FuncQrRenderer: Renderer<FuncQrOptions> = {
    ...
    listPoints: (props) => {
        ...
        if (funcType === FuncType.FuncA && type === Type.Rect) {
            pointList.push(
                `<circle fill='none' stroke-width='${nCount / 15
                }' stroke='${otherColor2}'  cx='${nCount / 2}' cy='${nCount / 2}' r='${((nCount / 2) * Math.sqrt(2) * 13) / 40
                }' />`,
            );
        }
            ...
            for (let y = 0; y < nCount; y++) {
                if (qrcode.isDark(x, y) && typeTable[x][y] === QRPointType.POS_CENTER) {
                    if (posType === PosType.Rect) {
                        ...
                        pointList.push(`<circle fill='${posColor}' cx='${x + 0.5}' cy='${y + 0.5}' r='${1.5}' />`);
                        pointList.push(
                            `<path d='${sq25}' stroke='${posColor}' stroke-width='${(100 / 6) * (1 - (1 - 0.8) * 0.75)
                    ...
                    if (funcType === FuncType.FuncA) {
                        ...
                        if (type === Type.Rect) {
                            sizeF = sizeF + 0.2;
                            pointList.push(
                                `<rect opacity='${opacityF}' width='${sizeF}' height='${sizeF}' fill='${colorF}' x='${x + (1 - sizeF) / 2
                                }' y='${y + (1 - sizeF) / 2}'/>`,
                            );
                        } else if (type === Type.Round) {
                            pointList.push(
                                `<circle opacity='${opacityF}' r='${sizeF}' fill='${colorF}' cx='${x + 0.5
                                }' cy='${y + 0.5}'/>`,
                            );
                        }
                    }
                    if (funcType === FuncType.FuncB) {
                        ...
                        if (type === Type.Rect) {
                            ...
                            if (qrcode.isDark(x, y)) {
                                ...
                                sizeF = sizeF - 0.1;
                                pointList.push(
                                    `<rect opacity='${opacityF}' width='${sizeF}' height='${sizeF}' stroke='${colorF}' stroke-width='${0.1}' fill='#FFFFFF' x='${x + (1 - sizeF) / 2
                                    }' y='${y + (1 - sizeF) / 2}'/>`,
                                );
                            }
                        } else if (type === Type.Round) {
                            if (qrcode.isDark(x, y)) {
                                ...
                                pointList.push(
                                    `<circle opacity='${opacityF}' r='${sizeF}' stroke='${colorF}' stroke-width='${0.1}' fill='#FFFFFF' cx='${x + 0.5
                                    }' cy='${y + 0.5}'/>`,
                                );
                            }
                        }
